...
export async function listNotes(req, res) {
  ...
  const notes = await Note.find(q).sort({ pinned: -1, updatedAt: -1 }).maxTimeMS(operationTimeoutMS()).lean();
  res.json({ data: notes });
}
export async function createNote(req, res) {
  ...
  recordActivitySoon(req.user._id, 'Created note', note.title, 'note', note._id);
